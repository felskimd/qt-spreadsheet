...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(std::function<CellInterface* (Position)> func) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<CellInterface* (Position)> func) const override {
                switch (type_) {
                case Add:
                    if (double temp = lhs_->Evaluate(func) + rhs_->Evaluate(func); std::isfinite(temp)) {
                        return temp;
                    }
                    else {
        ...
        public:
            ...
            double Evaluate(std::function<CellInterface* (Position)> func) const override {
                ...
                auto result = func(*cell_);
                if (!result) {
                    return 0.;
                }
                auto value = result->GetValue();
                if (const FormulaError* fe = std::get_if<FormulaError>(&value)) {
                    throw fe;
                }
                ...
                else if (const double* dbl = std::get_if<double>(&value)) {
                    return *dbl;
                }
                else return 0.;
            }
...
double FormulaAST::Execute(const SheetInterface& sheet) const {
    return root_expr_->Evaluate([&sheet](Position pos) { return (CellInterface*)sheet.GetCell(pos); });
}
